	...
	while (z<z_max)
	{
		...
		for (unsigned int i = 0; i < szL; i++)
		{
			...
			if (angle > 110.0)
			{
				...
				{
					zmin_prev = m_vecpLayers[index - 1]->ZminofLayer();
				}
				...
				CPoint3D layer_point = CPoint3D(m_vecpLayers[index]->m_vecpBoundaries[0]->m_vecpSegments[i]->m_ptStart);
				//layer_point = layer_point + m_vecpLayers[index]->m_vLayerCoordinate[2] * (-dz);
